...
/* [P, Q] = simonfunkMFGL(maxiter, lambda, alpha, Pini, Qini, train, B, lrate, Uknn, Mknn)     */
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 10) {
        mexErrMsgTxt("Ten inputs are required.");
    }
    if (nlhs != 2) {
    ...
    mwIndex *mkNNIr = mxGetIr(prhs[9]);
    mwIndex *mkNNJc = mxGetJc(prhs[9]);
    double *mkNN    = mxGetPr(prhs[9]);
